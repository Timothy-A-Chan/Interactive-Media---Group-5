...
if (index < temperature.getRowCount() && paused == false) {      // index goes up to 2010
    ...
    gl1.setValue(0.1f * Volume);
    if (dayCounter > 143 & dayCounter < 285) {//night time
      gl2.setValue(0.2f * Volume);
      gl3.setValue(0.5f * Volume);
    } else {//day time
...
public void hover(int currentTemp, float currentRainfall, int currentPeople) {
  int fillColor;
  int textX = 0;
  int textY = 0;
...
public void timeSlider() {
  ...
  if (currentDay == 1) fill(0xffFF0000);
  else if (currentDay == 2) fill(0xffF2E756);
  else if (currentDay == 3) fill(0xffE189C9);
  else if (currentDay == 4) fill(0xff44D24D);
  else if (currentDay == 5) fill(0xffCC46D5);
  else if (currentDay == 6) fill(0xffE4870E);
  else if (currentDay == 7) fill(0xff476ADC);
...
public void resetDay(int newIndex, int newDay) {
  index = newIndex;
  currentDay = newDay;
  dayCounter = 0;
}
...
public void drawBuilding(float skyHue, float buildingHue) {  //Top Left Starting Point: (80, 100);
  stroke(0, 0, 0);
...
public void drawBuildingOpensLarge(int X, int Y, float windowHue) {//input parameter only specifies position
...
public void drawBuildingOpensSmall(int X, int Y, float windowHue) {//same as above but smaller
...
public void drawWindow(float windowHue, float skyHue, float buildingHue) {//input value represents RGB value and skyHue for background
...
public void peopleCounterGenerator() {
...
public void peopleCounterPause(){
